...
function Agendamentos() {
  ...
  async function criarAgendamento(e) {
    ...
    if (!response.ok) {
      ...
      alert(errorData.error || t("agendamentosPage.saveError"));
      return;
    }
    setClienteId(""); setServicoIds([]); setData(""); setHorario(""); setEditingId(null);
    carregarAgendamentos();
  }
  ...
  return (
    <div>
      <form onSubmit={criarAgendamento} className="bg-white p-6 rounded shadow mb-6">
        <div className="mb-4">
          <label className="block mb-1">{t("agendamentosPage.client")}</label>
          <select value={clienteId} onChange={(e) => setClienteId(e.target.value)} className="w-full border p-2 rounded">
            <option value="">{t("agendamentosPage.selectClient")}</option>
        ...
        <div className="mb-4">
          <label className="block mb-1">{t("agendamentosPage.services")}</label>
              ...
              <label key={servico.id} className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setServicoIds(e.target.checked ? [...servicoIds, id] : servicoIds.filter((s) => s !== id));
                  }}
                  className="w-4 h-4"
                />
                <span>{servico.nome} - R$ {Number(servico.preco).toFixed(2)}</span>
              </label>
            ))}
          </div>
        </div>
        <div className="mb-4">
          <label className="block mb-1">{t("agendamentosPage.date")}</label>
          <input type="date" value={data} onChange={(e) => setData(e.target.value)} className="w-full border p-2 rounded" />
        </div>
        <div className="mb-4">
          <label className="block mb-1">{t("agendamentosPage.time")}</label>
        ...
        <div className="flex items-center gap-2">
          <button type="submit" className="bg-black text-white px-4 py-2 rounded">
            {editingId ? t("agendamentosPage.save") : t("agendamentosPage.create")}
          </button>
          {editingId && (
            <button type="button" onClick={() => { setEditingId(null); setClienteId(""); setServicoIds([]); setData(""); setHorario(""); }} className="bg-gray-300 text-black px-4 py-2 rounded">
              {t("agendamentosPage.cancel")}
            </button>
          )}
        </div>
      </form>
      ...
      <div className="space-y-4">
        {agendamentos.map((agendamento) => (
          <div key={agendamento.id} className="bg-white p-4 rounded shadow">
            ...
            <p>{t("agendamentosPage.servicesLabel")} {agendamento.servicos && agendamento.servicos.length > 0 ? agendamento.servicos.map((s) => s.nome).join(", ") : "—"}</p>
            <p>{t("agendamentosPage.value")} R$ {Number(agendamento.total).toFixed(2)}</p>
            <p>{t("agendamentosPage.dateLabel")} {formatDateBR(agendamento.data)}</p>
            <p>{t("agendamentosPage.timeLabel")} {String(agendamento.horario).slice(0, 5)}</p>
            <div className="mt-3 space-x-2">
              ...
              >
                {t("agendamentosPage.edit")}
              </button>
              <button onClick={() => deletarAgendamento(agendamento.id)} className="bg-red-500 text-white px-4 py-2 rounded">
                {t("agendamentosPage.delete")}
              </button>
            </div>
          </div>
        ))}
